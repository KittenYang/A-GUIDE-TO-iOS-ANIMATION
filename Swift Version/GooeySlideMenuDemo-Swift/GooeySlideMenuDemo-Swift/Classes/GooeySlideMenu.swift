...
class GooeySlideMenu: UIView {
    ...
    override func drawRect(rect: CGRect) {
        ...
        path.addQuadCurveToPoint(CGPoint(x: frame.width-_option.menuBlankWidth, y: frame.height), controlPoint: CGPoint(x: frame.width/2+diff, y: frame.height/2))
...
extension GooeySlideMenu {
    ...
    private func addButton() {
        ...
        if titles.count % 2 == 0 {
            ...
            for i in 0..<titles.count {
                ...
                home_button.bounds = CGRectMake(0, 0, frame.width - 20*2, _option.buttonHeight);
                addSubview(home_button)
                if (i >= titles.count / 2) {
                    index_up++
                    let y = frame.height/2 + _option.buttonHeight*CGFloat(index_up) + _option.buttonSpace*CGFloat(index_up)
                    home_button.center = CGPoint(x: frame.width/2, y: y+_option.buttonSpace/2 + _option.buttonHeight/2)
                } else {
                    index_down--
            ...
            for i in 0..<titles.count {
                ...
                let home_button = SlideMenuButton(option: buttonOption)
                home_button.bounds = CGRect(x: 0, y: 0, width: frame.width - 20*2, height: _option.buttonHeight)
                home_button.center = CGPoint(x: frame.width/2, y: frame.height/2 - _option.buttonHeight*CGFloat(index) - 20*CGFloat(index))
                addSubview(home_button)
            }
        }
    }
   ...
   @objc private func tapToUntrigger() {
        UIView.animateWithDuration(0.3) { [weak self] () -> Void in
            self?.frame = CGRect(x: -frame.width - _option.menuBlankWidth, y: 0, width: frame.width + _option.menuBlankWidth, height: frame.height)
        }
        ...
        UIView.animateWithDuration(0.7, delay: 0.0, usingSpringWithDamping: 0.6, initialSpringVelocity: 0.9, options: [.BeginFromCurrentState,.AllowUserInteraction], animations: { () -> Void in
            self.helperCenterView.center = CGPoint(x: -self.helperCenterView.frame.height/2, y: self.helperCenterView.frame.height/2)
